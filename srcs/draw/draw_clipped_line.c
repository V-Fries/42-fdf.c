...
/*   Updated: 2022/12/06 22:54:01 by vfries           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector_d	adjust_vector_to_screen(t_vector_d vector)
{
	vector = vector_divide(&vector, vector.w);
	vector.x = (vector.x + 1.0) * WINDOW_X / 2 + 0.5;
	vector.y = (vector.y + 1.0) * WINDOW_Y / 2 + 0.5;
	return (vector);
}
										...
										t_fdf *fdf, int color)
{
	...
	start = adjust_vector_to_screen(start);
	end = adjust_vector_to_screen(end);
